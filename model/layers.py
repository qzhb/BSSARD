...
class XCQAttention(nn.Module):
    def __init__(self, dim, drop_rate=0.0):
        super(XCQAttention, self).__init__()
        w4C_CQ = torch.empty(dim, 1)
        w4Q_CQ = torch.empty(dim, 1)
        w4mlu_CQ = torch.empty(1, 1, dim)
        nn.init.xavier_uniform_(w4C_CQ)
        nn.init.xavier_uniform_(w4Q_CQ)
        nn.init.xavier_uniform_(w4mlu_CQ)
        self.w4C_CQ = nn.Parameter(w4C_CQ, requires_grad=True)
        self.w4Q_CQ = nn.Parameter(w4Q_CQ, requires_grad=True)
        self.w4mlu_CQ = nn.Parameter(w4mlu_CQ, requires_grad=True)

        w4C_CX = torch.empty(dim, 1)
        w4X_CX = torch.empty(dim, 1)
        w4mlu_CX = torch.empty(1, 1, dim)
        nn.init.xavier_uniform_(w4C_CX)
        nn.init.xavier_uniform_(w4X_CX)
        nn.init.xavier_uniform_(w4mlu_CX)
        self.w4C_CX = nn.Parameter(w4C_CX, requires_grad=True)
        self.w4X_CX = nn.Parameter(w4X_CX, requires_grad=True)
        self.w4mlu_CX = nn.Parameter(w4mlu_CX, requires_grad=True)

        w4X_QX = torch.empty(dim, 1)
        w4Q_QX = torch.empty(dim, 1)
        w4mlu_QX = torch.empty(1, 1, dim)
        nn.init.xavier_uniform_(w4X_QX)
        nn.init.xavier_uniform_(w4Q_QX)
        nn.init.xavier_uniform_(w4mlu_QX)
        self.w4X_QX = nn.Parameter(w4X_QX, requires_grad=True)
        self.w4Q_QX = nn.Parameter(w4Q_QX, requires_grad=True)
        self.w4mlu_QX = nn.Parameter(w4mlu_QX, requires_grad=True)



        self.dropout = nn.Dropout(p=drop_rate)
        self.cqa_linear = Conv1D(in_dim=10 * dim, out_dim=dim, kernel_size=1, stride=1, padding=0, bias=True)

    def forward(self, context, query, cross, c_mask, q_mask):
        CQ_score = self.trilinear_attention_CQ(context, query)  # (batch_size, c_seq_len, q_seq_len)
        CQ_score_ = nn.Softmax(dim=2)(mask_logits(CQ_score, q_mask.unsqueeze(1)))  # (batch_size, c_seq_len, q_seq_len)
        CQ_score_t = nn.Softmax(dim=1)(mask_logits(CQ_score, c_mask.unsqueeze(2)))  # (batch_size, c_seq_len, q_seq_len)
        CQ_score_t = CQ_score_t.transpose(1, 2)  # (batch_size, q_seq_len, c_seq_len)
        CQ_c2q = torch.matmul(CQ_score_, query)  # (batch_size, c_seq_len, dim)
        CQ_q2c = torch.matmul(torch.matmul(CQ_score_, CQ_score_t), context)  # (batch_size, c_seq_len, dim)

        CX_score = self.trilinear_attention_CX(context, cross)  # (batch_size, c_seq_len, c_seq_len)
        CX_score_ = nn.Softmax(dim=2)(mask_logits(CX_score, c_mask.unsqueeze(1)))  # (batch_size, c_seq_len, c_seq_len)
        CX_score_t = nn.Softmax(dim=1)(mask_logits(CX_score, c_mask.unsqueeze(2)))  # (batch_size, c_seq_len, c_seq_len)
        CX_score_t = CX_score_t.transpose(1, 2)  # (batch_size, c_seq_len, c_seq_len)
        CX_c2x = torch.matmul(CX_score_, cross)  # (batch_size, c_seq_len, dim)
        CX_x2c = torch.matmul(torch.matmul(CX_score_, CX_score_t), context)  # (batch_size, c_seq_len, dim)

        QX_score = self.trilinear_attention_QX(cross, query)  # (batch_size, c_seq_len, c_seq_len)
        QX_score_ = nn.Softmax(dim=2)(mask_logits(QX_score, q_mask.unsqueeze(1)))  # (batch_size, c_seq_len, c_seq_len)
        QX_score_t = nn.Softmax(dim=1)(mask_logits(QX_score, c_mask.unsqueeze(2)))  # (batch_size, c_seq_len, c_seq_len)
        QX_score_t = QX_score_t.transpose(1, 2)  # (batch_size, c_seq_len, c_seq_len)
        QX_x2q = torch.matmul(QX_score_, query)  # (batch_size, c_seq_len, dim)
        QX_q2x = torch.matmul(torch.matmul(QX_score_, QX_score_t), cross)  # (batch_size, c_seq_len, dim)


        output = torch.cat([context,
                            CQ_c2q, torch.mul(context, CQ_c2q), torch.mul(context, CQ_q2c),
                            CX_c2x, torch.mul(context, CX_c2x), torch.mul(context, CX_x2c),
                            QX_x2q, torch.mul(cross, QX_x2q), torch.mul(cross, QX_q2x),
                            ], dim=2)
        output = self.cqa_linear(output)  # (batch_size, c_seq_len, dim)
        return output

    def trilinear_attention_CQ(self, context, query):
        batch_size, c_seq_len, dim = context.shape
        batch_size, q_seq_len, dim = query.shape
        context = self.dropout(context)
        query = self.dropout(query)
        subres0 = torch.matmul(context, self.w4C_CQ).expand([-1, -1, q_seq_len])  # (batch_size, c_seq_len, q_seq_len)
        subres1 = torch.matmul(query, self.w4Q_CQ).transpose(1, 2).expand([-1, c_seq_len, -1])
        subres2 = torch.matmul(context * self.w4mlu_CQ, query.transpose(1, 2))
        res = subres0 + subres1 + subres2  # (batch_size, c_seq_len, q_seq_len)
        return res

    def trilinear_attention_CX(self, context, cross):
        batch_size, c_seq_len, dim = context.shape
        batch_size, x_seq_len, dim = cross.shape
        context = self.dropout(context)
        cross = self.dropout(cross)
        subres0 = torch.matmul(context, self.w4C_CX).expand([-1, -1, x_seq_len])  # (batch_size, c_seq_len, q_seq_len)
        subres1 = torch.matmul(cross, self.w4X_CX).transpose(1, 2).expand([-1, c_seq_len, -1])
        subres2 = torch.matmul(context * self.w4mlu_CX, cross.transpose(1, 2))
        res = subres0 + subres1 + subres2  # (batch_size, c_seq_len, q_seq_len)
        return res

    def trilinear_attention_QX(self, cross, query):
        batch_size, c_seq_len, dim = cross.shape
        batch_size, q_seq_len, dim = query.shape
        cross = self.dropout(cross)
        query = self.dropout(query)
        subres0 = torch.matmul(cross, self.w4X_QX).expand([-1, -1, q_seq_len])  # (batch_size, c_seq_len, q_seq_len)
        subres1 = torch.matmul(query, self.w4Q_QX).transpose(1, 2).expand([-1, c_seq_len, -1])
        subres2 = torch.matmul(cross * self.w4mlu_QX, query.transpose(1, 2))
        res = subres0 + subres1 + subres2  # (batch_size, c_seq_len, q_seq_len)
        return res
...
class HighLightLayer(nn.Module):
    ...
    def compute_loss(scores, labels, mask, epsilon=1e-12):
        ...
        weights = torch.where(labels == 0.0, labels + 1, 2.0 * labels)
        # weights = torch.where(labels == 0.0, labels + labels.sum() / torch.count_nonzero(labels), 2.0 * labels)
...
class ConditionedPredictor(nn.Module):
    ...
    def forward(self, x, mask):
        context_feat = self.average_mask(x, mask)
        bias_logits = self.discrim(context_feat)
        # context_feat = self.discrim2(x).transpose(1, 2)
        # context_feat = self.pool(context_feat).squeeze()
        # bias_logits = self.fc(context_feat)
        ...
        return start_logits, end_logits, bias_logits
    ...
    def compute_cross_entropy_loss(start_logits, end_logits, start_labels, end_labels):
        ...
        start_loss = - (start_logits.log_softmax(-1) * start_labels).sum(1).mean(0)
        end_loss = - (end_logits.log_softmax(-1) * end_labels).sum(1).mean(0)

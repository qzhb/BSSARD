...
class Z2Q_128x128(nn.Module):
    def __init__(self):
        super(Z2Q_128x128, self).__init__()
    ...
    def forward(self, z_ct, z_p):
        ...
        word_feats = self.conv4(word_feats)
        word_feats = self.conv5(word_feats)
        word_feats = self.conv6(word_feats)
        out = self.conv7(word_feats).squeeze(dim=1)
        return out

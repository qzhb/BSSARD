...
def convert_length_to_mask2(lengths, max_len):
    mask = torch.arange(max_len, device=lengths.device).expand(lengths.size()[0], max_len) < lengths.unsqueeze(1)
    mask = mask.float()
    return mask
...
def calculate_diou_accuracy(ious, threshold, deltas):
    total_size = float(len(ious))
    count = 0
    for iou, delta in zip(ious, deltas):
        if iou >= threshold:
            delta_s, delta_e = delta
            count += 1 * delta_s * delta_e
    return float(count) / total_size * 100.0
...
def eval_test(model, data_loader, device, mode='test', epoch=None, global_step=None):
    ious = []
    with torch.no_grad():
        for idx, (records, vfeats, vfeat_lens, word_ids, char_ids) in enumerate(data_loader):
            # prepare features
            ...
            for record, start_index, end_index in zip(records, start_indices, end_indices):
                if start_index >= record["v_len"]:
                    start_index=record["v_len"] - 1
                if end_index >= record["v_len"]:
                    end_index=record["v_len"] - 1

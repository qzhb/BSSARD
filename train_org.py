...
val_loader = get_test_loader(dataset['val_set'], configs)
test_loader = get_test_loader(dataset=dataset['test_set'], configs=configs)
...
for epoch in range(configs.epochs):
    ...
    # 测试一轮
    model.eval()
    r1i3, r1i5, r1i7, dr1i3, dr1i5, dr1i7, mIoU, score_str = model_adapter.test_one_epoch_template(test_loader, "test")
    print(
        '\n【Test】Epoch: %2d | Step: %5d | r1i3: %.2f | r1i5: %.2f | r1i7: %.2f \ndr1i3: %.2f | dr1i5: %.2f | dr1i7: %.2f \nmIoU: %.2f' % (
            epoch + 1, (epoch + 1) * num_train_batches, r1i3, r1i5, r1i7, dr1i3, dr1i5, dr1i7, mIoU), flush=True)
    if configs.verbose:
        score_writer.write(score_str + f"(test)\n")
        score_writer.flush()
        writer.add_scalar(f'test_score/r1i3', r1i3, epoch + 1)
        writer.add_scalar(f'test_score/r1i5', r1i5, epoch + 1)
        writer.add_scalar(f'test_score/r1i7', r1i7, epoch + 1)
        writer.add_scalar(f'test_score/mIoU', mIoU, epoch + 1)
    best_r1i5 = max(best_r1i5, r1i5)
    best_dr1i5 = max(best_dr1i5, dr1i5)
    best_dr1i7 = max(best_dr1i7, dr1i7)
    best_mIoU = max(best_mIoU, mIoU)
    model.train()
    if r1i7 >= best_r1i7 and configs.verbose:
        best_r1i7 = r1i7
